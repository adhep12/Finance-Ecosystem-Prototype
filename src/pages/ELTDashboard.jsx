...
// ─────────────────────────────────────────────────────────────────────────────
// ELT Date Range Picker — includes fiscal + operating year presets
// ─────────────────────────────────────────────────────────────────────────────

function ELTDateRangePicker({ dateRange, org, onApplyPreset, onApplyCustom, onClose, accent }) {
  const [localStart, setLocalStart] = useState(dateRange.startDate)
  const [localEnd,   setLocalEnd]   = useState(dateRange.endDate)

  const fy   = org.fiscalYearStartMonth
  const fyY  = org.fiscalYearStartYear
  const oy   = org.operatingYearStartMonth
  const oyY  = org.operatingYearStartYear

  const fyeY = fy === 1 ? fyY : fyY + 1
  const fyeM = fy === 1 ? 12 : fy - 1
  const oyeY = oy === 1 ? oyY : oyY + 1
  const oyeM = oy === 1 ? 12 : oy - 1

  const fyLabel  = `${monthName(fy)} ${fyY} → ${monthName(fyeM)} ${fyeY}`
  const fytLabel = `${monthName(fy)} ${fyY} → Today`
  const oyLabel  = `${monthName(oy)} ${oyY} → ${monthName(oyeM)} ${oyeY}`
  ...
  function handleApply() {
    if (localStart && localEnd && localStart <= localEnd) {
      onApplyCustom(localStart, localEnd); onClose()
    }
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-2xl border border-gray-200 p-5 w-80">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        {btn('full-fiscal', 'Full fiscal year', fyLabel)}
        {btn('fiscal-ytd',  'Fiscal YTD',       fytLabel)}
      </div>

      <div className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 mb-2">Operating Year</div>
      <div className="grid grid-cols-2 gap-2 mb-4">
        {btn('full-operating', 'Full operating year', oyLabel)}
        {btn('operating-ytd',  'Operating YTD',       oytLabel)}
      </div>
...
function ELTNav({ orgConfig, activeTab, setActiveTab, dateRange, onApplyPreset, onApplyCustom }) {
  ...
  const accent    = orgConfig.accentColor || '#0EA5A0'
...
// ─────────────────────────────────────────────────────────────────────────────
// Net Position Card (hover breakdown)
// ─────────────────────────────────────────────────────────────────────────────
        ...
        {[
          ...
          <div key={label}>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`text-sm font-semibold ${varColor(delta)}`}>
...
// ─────────────────────────────────────────────────────────────────────────────
// Generic Add Card Panel (shared by KPI + Patron sections)
// ─────────────────────────────────────────────────────────────────────────────

function AddCardPanel({ title, suggestedCards, existingIds, onAdd, onClose, accent }) {
          ...
          {mode === 'suggested' ? (
            <div className="space-y-2">
              ...
              {available.map(card => (
                <button key={card.id} onClick={() => { onAdd(card); onClose() }}
                  className="w-full text-left px-4 py-3 rounded-xl border border-gray-200 hover:bg-gray-50 transition-all group">
                  <div className="flex items-center justify-between">
                    <div>
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full py-2 rounded-lg text-sm font-medium text-white disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
                style={{ backgroundColor: accent }}>
                Add Card
              </button>
            </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.map((row, i) => {
              ...
              const pctOfIncome = (row.actual ?? 0) / totalIncome * 100
              ...
              return (
                ...
                `}>
                  <td className={`px-6 py-2.5
                    ${isSection  ? 'text-[10px] font-bold uppercase tracking-widest text-gray-400' : ''}
                  ...
                  {isSection ? <td colSpan={4} /> : (
                    <>
                      <td className={`px-4 py-2.5 text-right tabular-nums font-medium
                        ${isTotal ? 'text-white' : isSubtotal ? 'text-gray-800' : 'text-gray-700'}`}>
...
// ─────────────────────────────────────────────────────────────────────────────
// Patron Metric Card — accepts raw delta numbers (no pre-formatted strings)
// ─────────────────────────────────────────────────────────────────────────────
...
function PatronMetricCard({ label, mainValue, sub1Label, sub1Delta, sub1Format, sub2Label, sub2Delta, sub2Format, editMode, onRemove }) {
  // sub1Format / sub2Format: 'count' | 'currency' | 'percent' | 'plain'
  function fmt(delta, format) {
    if (delta === null || delta === undefined) return '—'
    const sign = delta > 0 ? '+' : delta < 0 ? '' : ''
    if (format === 'currency') return (delta > 0 ? '+' : '') + formatCurrency(delta)
    if (format === 'percent')  return formatPercent(delta, { showSign: true, decimals: 1 })
    if (format === 'count')    return (delta > 0 ? '+' : '') + delta.toLocaleString()
      ...
      <div className="space-y-1.5">
        {sub1Label && (
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`text-xs font-semibold tabular-nums ${varColor(sub1Delta)}`}>
              {fmt(sub1Delta, sub1Format)}
            </span>
          </div>
        )}
        {sub2Label && (
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`text-xs font-semibold tabular-nums ${varColor(sub2Delta)}`}>
              {fmt(sub2Delta, sub2Format)}
            </span>
          </div>
        )}
      </div>
...
// Patron chart cards — removable wrappers
function NewPatronChartCard({ data, accent, editMode, onRemove }) {
  return (
    <div className="relative bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      {editMode && onRemove && (
        <button onClick={onRemove}
          className="absolute top-2 right-2 w-5 h-5 rounded-full bg-gray-100 hover:bg-red-100 text-gray-400 hover:text-red-500 flex items-center justify-center transition-colors z-10">
          <X size={11} />
        </button>
      )}
      ...
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
          ...
          <Line type="monotone" dataKey="newCY" name="This Year"  stroke={accent} strokeWidth={2} dot={false} />
...
function PatronBaseChartCard({ data, accent, editMode, onRemove }) {
  return (
    <div className="relative bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      {editMode && onRemove && (
        <button onClick={onRemove}
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
          ...
          <Bar dataKey="total" name="Total Patrons" fill={accent} radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
...
// Suggested patron cards catalogue
...
// ─────────────────────────────────────────────────────────────────────────────
// Suggested KPI Card rendering (from data)
// ─────────────────────────────────────────────────────────────────────────────
...
function SuggestedKPICard({ cardId, expenseData, patronData, editMode, onRemove }) {
  ...
  if (cardId === 'top-expense') {
    const top = Object.entries(expenseData).sort((a, b) => b[1] - a[1])[0]
    const names = { staff: 'Staff', contract: 'Contract', technology: 'Technology', travel: 'Travel', otherGenAdmin: 'Other G&A' }
    label = 'Top Expense Category'
    value = names[top[0]] || top[0]
    sub   = formatCurrency(top[1])
    ...
    value = `${((totalExp / totalBudget) * 100).toFixed(1)}%`
    sub   = `${formatCurrency(totalExp)} of ${formatCurrency(totalBudget)}`
    ...
    sub   = `vs ${((patronData.priorYear / patronData.priorYear) * 100).toFixed(1)}% baseline`
  } else {
    label = cardId; value = '—'
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Edit section header (shared)
// ─────────────────────────────────────────────────────────────────────────────

function SectionHeader({ title, editMode, onToggleEdit, onAdd, accent }) {
  return (
...
// storage for manually added cards with their values
const manualCardStore = {}

function DashboardTab({ dateRange, accent }) {
  ...
  function renderKPICard(cardId) {
    ...
    if (cardId === 'net-position') {
      ...
      return <NetPositionCard key={cardId}
        value={netPosition}
    ...
    if (SUGGESTED_KPI_CARDS.find(s => s.id === cardId)) {
      return <SuggestedKPICard key={cardId} cardId={cardId}
        expenseData={d.expenseLines} patronData={d.patrons}
        editMode={editKPI} onRemove={() => setKpiCards(p => p.filter(c => c !== cardId))} />
    }
    // manual
    const stored = manualCardStore[cardId] || { label: cardId, value: '—' }
    return <ManualKPICard key={cardId} card={stored}
      editMode={editKPI} onRemove={() => setKpiCards(p => p.filter(c => c !== cardId))} />
  }

  // ── Patron card renderer ──
  function renderPatronCard(cardId) {
    const p = d.patrons
    if (cardId === 'total-patrons') {
      return <PatronMetricCard key={cardId}
        label="Total Patrons" mainValue={p.total.toLocaleString()}
        sub1Label="vs Prior Month" sub1Delta={p.total - p.priorMonth}  sub1Format="count"
        sub2Label="vs Prior Year"  sub2Delta={p.total - p.priorYear}   sub2Format="count"
        editMode={editPatron} onRemove={() => setPatronCards(c => c.filter(x => x !== cardId))} />
    }
    if (cardId === 'new-patrons') {
      const growthPct = (p.newThisPeriod / p.newPriorPeriod - 1) * 100
      return <PatronMetricCard key={cardId}
        label="New Patrons (Period)" mainValue={p.newThisPeriod.toLocaleString()}
        sub1Label="vs Prior Period"  sub1Delta={p.newThisPeriod - p.newPriorPeriod}  sub1Format="count"
        sub2Label="Growth rate"      sub2Delta={growthPct}                            sub2Format="percent"
        editMode={editPatron} onRemove={() => setPatronCards(c => c.filter(x => x !== cardId))} />
    }
    if (cardId === 'avg-gift') {
      return <PatronMetricCard key={cardId}
        label="Avg Gift Size" mainValue={`$${p.avgGift.toFixed(2)}`}
        sub1Label="vs Prior Year"  sub1Delta={p.avgGift - p.avgGiftPriorYear}  sub1Format="currency"
        sub2Label="Prior year avg" sub2Delta={null}                             sub2Format="plain"
        editMode={editPatron} onRemove={() => setPatronCards(c => c.filter(x => x !== cardId))}>
        {/* override sub2 to show fixed label */}
      </PatronMetricCard>
    }
    if (cardId === 'new-patron-chart') {
      return <NewPatronChartCard key={cardId} data={p.monthly} accent={accent}
        editMode={editPatron} onRemove={() => setPatronCards(c => c.filter(x => x !== cardId))} />
    }
    if (cardId === 'patron-base-chart') {
      return <PatronBaseChartCard key={cardId} data={p.base} accent={accent}
        editMode={editPatron} onRemove={() => setPatronCards(c => c.filter(x => x !== cardId))} />
    }
    // Manual patron card
    const stored = manualCardStore[cardId] || { label: cardId, value: '—' }
  ...
  // Split patron metric cards from chart cards for layout
  const patronMetricIds = patronCards.filter(id =>
    ['total-patrons','new-patrons','avg-gift'].includes(id) ||
    id.startsWith('manual-') ||
    !['new-patron-chart','patron-base-chart'].includes(id)
  )
  const patronChartIds = patronCards.filter(id =>
    ['new-patron-chart','patron-base-chart'].includes(id)
  )
    ...
    <div className="p-6 space-y-8 max-w-screen-xl mx-auto">
      ...
      <section>
        <SectionHeader title="Key Performance Indicators"
          editMode={editKPI} onToggleEdit={() => setEditKPI(v => !v)}
          onAdd={() => setShowAddKPI(true)} accent={accent} />
        <div className="flex gap-4 flex-wrap">
          {kpiCards.map(id => renderKPICard(id))}
          {editKPI && (
            <button onClick={() => setShowAddKPI(true)}
              className="flex flex-col items-center justify-center gap-2 bg-white rounded-2xl border-2 border-dashed border-gray-200 hover:border-gray-400 transition-all p-5 min-w-[160px] text-gray-300 hover:text-gray-500">
              <Plus size={20} />
      ...
      {/* ── Patron Composition ── */}
      <section>
        <SectionHeader title="Patron Composition"
          editMode={editPatron} onToggleEdit={() => setEditPatron(v => !v)}
          onAdd={() => setShowAddPatron(true)} accent={accent} />
        ...
        {patronMetricIds.length > 0 && (
          <div className="flex gap-4 flex-wrap mb-5">
            ...
            {editPatron && (
              <button onClick={() => setShowAddPatron(true)}
                className="flex flex-col items-center justify-center gap-2 bg-white rounded-2xl border-2 border-dashed border-gray-200 hover:border-gray-400 transition-all p-5 min-w-[160px] text-gray-300 hover:text-gray-500">
                <Plus size={20} />
                <span className="text-xs font-medium">Add card</span>
              </button>
        ...
        {patronCards.length === 0 && (
          <div className="text-center py-12 text-gray-400 text-sm">
            No patron cards. Click <strong>Edit</strong> → <strong>Add card</strong> to add one.
          </div>
        )}
      </section>
      ...
      {/* Add panels */}
      {showAddKPI && (
        <AddCardPanel title="Add KPI Card"
          suggestedCards={SUGGESTED_KPI_CARDS}
          existingIds={kpiCards}
          accent={accent}
          onAdd={card => {
            if (card.manual) manualCardStore[card.id] = card
            setKpiCards(p => [...p, card.id])
          }}
          onClose={() => setShowAddKPI(false)} />
      )}
      {showAddPatron && (
        <AddCardPanel title="Add Patron Card"
          suggestedCards={SUGGESTED_PATRON_CARDS}
          existingIds={patronCards}
          accent={accent}
          onAdd={card => {
            if (card.manual) manualCardStore[card.id] = card
            setPatronCards(p => [...p, card.id])
          }}
          onClose={() => setShowAddPatron(false)} />
      )}
    </div>
  )
}
...
function MonthlySummaryTab({ accent }) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-start gap-4 mb-6 flex-wrap">
        <h2 className="text-sm font-semibold text-gray-900 pt-1.5">Monthly Narrative</h2>
        <div className="flex items-center gap-1 bg-gray-100 rounded-full p-1 flex-wrap">
          {months.map(m => (
          ...
          <tbody>
            {teams.map(t => {
              ...
              return (
                <tr key={t.dept} className="border-b border-gray-50 hover:bg-gray-50 transition-colors cursor-pointer group">
                  <td className="px-6 py-3 font-medium text-gray-800 group-hover:text-gray-900">
                    <div className="flex items-center gap-2">
                      {t.name}
                  ...
                  <td className="px-4 py-3 text-right tabular-nums font-medium text-gray-700">
                    {formatCurrency(t.actual, { compact: false })}
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums text-gray-500">
                    {formatCurrency(t.budget, { compact: false })}
                  </td>
                  ...
                  <td className="px-6 py-3 text-right tabular-nums text-gray-400 text-xs">
                    {formatPercent((t.actual / totalActual) * 100, { decimals: 1 })}
                  </td>
                </tr>
              )
            })}
            <tr className="bg-gray-900">
              ...
              <td className="px-4 py-3 text-right tabular-nums text-gray-400">
                {formatCurrency(totalBudget, { compact: false })}
              </td>
              <td className={`px-4 py-3 text-right tabular-nums font-bold ${totalActual - totalBudget <= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {(() => { const v = totalActual - totalBudget; return (v > 0 ? '+' : '') + formatCurrency(v, { compact: false }) })()}
              </td>
...
export default function ELTDashboard() {
  ...
  function applyPreset(preset) {
    setDateRange({ preset, ...getELTPresetRange(preset, orgConfig) })
  }
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--color-primary-bg)' }}>
      ...
      <main className="flex-1 overflow-auto">
        {activeTab === 'dashboard' && <DashboardTab  dateRange={dateRange} accent={accent} />}
        {activeTab === 'summary'   && <MonthlySummaryTab accent={accent} />}
        {activeTab === 'teams'     && <TeamsTab />}
        {activeTab === 'documents' && <DocumentsTab accent={accent} />}
        {activeTab === 'import'    && <ELTImportTab accent={accent} />}
      </main>
    </div>
  )
}
